...
class Item extends BladeComponent
{
    public function render()
    {
        return <<<'BLADE'
            ...
            $iconClasses = TALLKit::classes($square ? 'size-5!' : 'size-4!');
        ...
        <tk:button :attributes="$attributes->class($classes)" data-flux-navlist-item>
            <?php if ($icon): ?>
                <div class="relative">
                    <?php if (is_string($icon) && $icon !== ''): ?>
                        <tk:icon :$icon :variant="$iconVariant" class="{!! $iconClasses !!}" />
                    <?php else: ?>
                        {{ $icon }}
                    <?php endif; ?>
            ...
            <?php if (is_string($iconTrailing) && $iconTrailing !== ''): ?>
                <tk:icon :icon="$iconTrailing" :variant="$iconVariant" class="size-4!" />
            <?php elseif ($iconTrailing): ?>
                {{ $iconTrailing }}
            <?php endif; ?>

            <?php if ($badge): ?>
                <tk:navlist.badge :color="$badgeColor">{{ $badge }}</tk:navlist.badge>
            <?php endif; ?>
        </tk:button>
        BLADE;
    }
