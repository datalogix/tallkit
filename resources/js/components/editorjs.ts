...
const GROUPS: Record<string, { scripts: string[], inline?: string[], tools(): Record<string, any> }> = {
  text: {
    scripts: [
...
function resolveGroups(mode?: string | null) {
  const groups = parseMode(mode, EDITOR_GROUP_ORDER) ?? EDITOR_GROUP_ORDER

  warnUnsupportedGroups('editorjs', groups, GROUPS)
