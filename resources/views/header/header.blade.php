    ...
    {{
        $attributes
            ...
            ->classes(match ($variant) {
                ...
                default => TALLKit::frameBackground($variant) ?? 'bg-zinc-700/10 dark:bg-zinc-800 [:where(&)]:text-zinc-800/85 dark:[:where(&)]:text-white/85',
            })
    }}
>
    <tk:container.wrapper
        :attributes="TALLKit::attributesAfter($attributes, 'container:')->classes('min-h-16 h-full flex items-center')"
        :$container
    >
        {{ $slot }}
    </tk:container.wrapper>
